...
use AbleSDK::Configuration;

sub new {
    my $class = shift;

    my $config;
    if ( $_[0] && ref $_[0] && ref $_[0] eq 'AbleSDK::Configuration' ) {
        $config = $_[0];
    } else {
        $config = AbleSDK::Configuration->new(@_);
    }
...
sub deserialize
{
    ...
    if (not defined $data) {
        ...
        my $_instance = use_module("AbleSDK::Object::$class")->new;
        if (ref $data eq "HASH") {

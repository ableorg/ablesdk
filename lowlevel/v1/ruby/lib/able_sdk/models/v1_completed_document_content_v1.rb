  ...
  class V1CompletedDocumentContentV1
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `AbleSDK::V1CompletedDocumentContentV1` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `AbleSDK::V1CompletedDocumentContentV1`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'status')
        ...
        self.status = 'DOCUMENT_CONTENT_STATUS_UNSPECIFIED'
      end
    ...
    def hash
      [document_content_id, status, last_error, acquired_by, acquired_time, completed_by, completed_time].hash
    end
